...
public class FourPointScheme {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		Point2D.Double[] points = new Point2D.Double[] { new Point2D.Double(100, 100), new Point2D.Double(400, 100),
		...
		for (int j = 0; j < 1; j++) {
			...
			for (int i = 0; i < 17; i++) {
				...
				// System.out.println("Maximum edge length ratio is " +
				// maxEdgeLengthRatio(points) + ".");
			}
	...
	private static void drawPoints(DrawFrame drawFrame, Double[] points) {
		...
		for (int i = 0; i < points.length - 1; i++) {
			g.drawLine((int) Math.round(points[i].x), (int) Math.round(points[i].y), (int) Math.round(points[i + 1].x),
	...
	public static Point2D.Double[] applyFourPointScheme(Point2D.Double[] source, double tension) {
		...
		for (int i = 3; i < subdivided.length - 2; i += 2) {
			subdivided[i] = sub(mul(add(source[i / 2], source[i / 2 + 1]), tension + 0.5),
					mul(add(source[i / 2 - 1], source[i / 2 + 2]), tension));
		}

		subdivided[1] = sub(mul(add(source[0], source[1]), tension + 0.5), mul(add(source[0], source[2]), tension));
		subdivided[subdivided.length - 2] = sub(
				mul(add(source[source.length - 2], source[source.length - 1]), tension + 0.5),
				mul(add(source[source.length - 3], source[source.length - 1]), tension));
	...
	public static Point2D.Double[] applyConvexityPreservingFourPointScheme(Point2D.Double[] source, double C, double W) {
		...
		for (int i = 1; i < subdivided.length; i += 2) {
			...
			// ////////////////// CHOOSE TANGENTS
			final double interpolationFactor = 1.0 / subdivided.length;

			if (i == 1) {
				tangentB = add(b, new Point2D.Double(b.y - c.y, c.x - b.x));
			} else {
				tangentB = add(b, add(mul(sub(c, b), 1 - interpolationFactor), mul(sub(b, a), interpolationFactor)));
			}

			if (i == subdivided.length - 1) {
				tangentC = add(c, new Point2D.Double(b.y - c.y, c.x - b.x));
			} else {
				tangentC = add(c, add(mul(sub(c, b), interpolationFactor), mul(sub(d, c), 1 - interpolationFactor)));
			}

//			if (i + 1 == subdivided.length / 2) {
//				System.out.println(interpolationFactor);
//
//				tangentC = add(c, add(mul(sub(c, b), 1 - interpolationFactor), mul(sub(d, c), interpolationFactor)));
//			} else if (i - 1 == subdivided.length / 2) {
//				System.out.println(interpolationFactor);
//
//				tangentB = add(b, add(mul(sub(c, b), interpolationFactor), mul(sub(b, a), 1 - interpolationFactor)));
//			}
			// ////////////////// TANGENTS CHOSEN
